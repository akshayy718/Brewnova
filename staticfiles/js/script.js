/**
 * Brewnova Cafe - Main JavaScript File
 * Handles all interactive functionality for the website
 */

// DOM Content Loaded Event
document.addEventListener('DOMContentLoaded', function() {
    console.log('Brewnova Cafe JavaScript Loaded!');
...
function initializeOrderForm() {
    ...
    function updateCategoryDisplay(category) {
        ...
        window.orderItems[category].forEach((item, index) => {
            ...
            itemDiv.innerHTML = `
                <div class="item-details">
                    ...
                    <span class="item-info">AED${item.price} × ${item.quantity} = AED${item.subtotal}</span>
                </div>
        ...
        subtotalSpan.textContent = `AED${categoryTotal}`;
    }
    ...
    function updateOrderSummary() {
            ...
            allItems.forEach(item => {
                summaryHTML += `
                    <div class="summary-item">
                        ...
                        <span class="item-price">AED${item.subtotal}</span>
                    </div>
                `;
            });
            summaryHTML += '</div>';
        ...
        // Update amounts
        if (subtotalAmount) subtotalAmount.textContent = `AED${subtotal}`;
        if (deliveryFee) {
            deliveryFee.textContent = delivery === 0 ? 'FREE' : `AED${delivery}`;
        }
        if (gstAmount) gstAmount.textContent = `AED${gst}`;
        if (totalAmount) totalAmount.innerHTML = `<strong>AED${total}</strong>`;
...
function setupFormValidation() {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (totalElement && totalElement.textContent === 'AED0') {
